...
//
//	client.Group.Query().
//		GroupBy(group.FieldDeletedTime).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (ggb *GroupGroupBy) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*GroupGroupBy](ctx, ggb, ggb.build.inters, v)
}

func (ggb *GroupGroupBy) sqlScan(ctx context.Context, v any) error {
	selector := ggb.sqlQuery(ctx)
	if err := selector.Err(); err != nil {
		return err
	}
	rows := &sql.Rows{}
...
func (ggb *GroupGroupBy) sqlQuery(ctx context.Context) *sql.Selector {
	selector := ggb.build.sqlQuery(ctx).Select()
	...
	return selector.GroupBy(selector.Columns(*ggb.flds...)...)
}
...
func (gs *GroupSelect) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*GroupSelect](ctx, gs, gs.inters, v)
}

func (gs *GroupSelect) sqlScan(ctx context.Context, v any) error {
	selector := gs.GroupQuery.sqlQuery(ctx)
